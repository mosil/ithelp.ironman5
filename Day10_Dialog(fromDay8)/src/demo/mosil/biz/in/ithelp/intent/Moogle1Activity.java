	...
	protected void onStart() {
		...
		try{
			...
			if(bundleFromMoogle2 != null){
		    	...
		    	dialog.setNegativeButton("重送", new DialogInterface.OnClickListener() {
		    		public void onClick(DialogInterface dialog, int which) {
		    			Intent intent = new Intent(Moogle1Activity.this, Moogle2Activity.class);
		    			int signal = (int)(Math.random()*10) + 1;
		    			Bundle bundle = new Bundle();
		    	...
		    	dialog.setNeutralButton("說法", new DialogInterface.OnClickListener() {
		    		public void onClick(DialogInterface dialog, int which) {
		    			Toast.makeText(Moogle1Activity.this, "勇者啊～放下屠刀立地成佛，咱們當個朋友吧～", Toast.LENGTH_LONG).show();
					}
				});
		        dialog.show();
			}
